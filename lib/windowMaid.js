...
const windowMaid = {
  ...
  registerOpenWindow (exportJob) {
    ...
    windowCache[w.id] = {
      ...
      window: w,
      lastUsed: Date.now(),
      // How long a window can remain open before being forcefully closed
      lifespan: windowLifespan // in milliseconds
    }
  },

  /**
   * Gets a cache entry for a window.  Can be used to get job correlated to the window.
   *
   * @param {Number} id

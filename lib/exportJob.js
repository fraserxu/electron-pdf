...
class ExportJob extends EventEmitter {
   ...
   *
   * @param {Object} [options.printPageSize] If provided, this page size is used
   * for printToPDF instead of args.pageSize.  This allows the caller to provide
   * page dimensions in units that printToPDF expects (inches in Electron 40+)
   * while keeping args.pageSize in the units that WindowTailor expects (microns)
   * for correct browser window sizing.
   *
  ...
  _capturePDF (args, window, done, outputFile) {
    ...
    const pdfOptions = {
      ...
      pageSize: this.options.printPageSize || args.pageSize,

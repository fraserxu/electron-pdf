...
class ExportJob extends EventEmitter {
   ...
   * @fires ExportJob#export-complete after each resource is available on the filesystem
  ...
  changeArgValue (arg, value) {
    this.args[arg] = value
    // let s = this.window.getSize()
    // this.window.setSize(s[1], s[0]) //change orientation
  }
  ...
  _captureImage (window, outputFile, done) {
    window.webContents.capturePage(image => {
      ...
      if (this.options.inMemory) {
        this.emit('window.capture.end', {data: pngBuffer})
        done(undefined, pngBuffer)
      } else {
        const target = path.resolve(outputFile)
        fs.writeFile(target, pngBuffer, function (err) {
          this.emit('window.capture.end', {data: target, error: err})
          this.emit('export-complete', {file: target})
          done(err, target)
        }.bind(this))
      }

...
class ExportJob extends EventEmitter {
  ...
  destroy () {
    if (this.window) {
      try {
        ...
        // window.close can throw "Object has been destroyed", lets ignore it for now
      } finally {
    ...
    const cmd = `var ipcRenderer = require('electron').ipcRenderer
                 document.addEventListener('${eventName}', 
                   function(event) {
                     ...
                     ipcRenderer.send('READY_TO_RENDER', event.detail)
                   }
                 )`
    // Don't let things hang forever
    const timeout = setTimeout(() => {
      this.emit('window.event.wait.timeout', {eventName: eventName})
      // TODO: Need to cache all pending jobs and correlate on JobId, the listener should stay active
  ...
  _attachIPCListener (eventName, generateFunction) {
    ...
    electron.ipcMain.on('READY_TO_RENDER', (name, customEventDetail) => {

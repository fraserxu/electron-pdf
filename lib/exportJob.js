...
class ExportJob extends EventEmitter {
  ...
  _getHeaders () {
    const headers = []
    this.args.acceptLanguage && headers.push(`Accept-Language: ${this.args.acceptLanguage}`)
  ...
  _load (win) {
    if (this.args.noprint) {
      ...
      this._renderAndCollectOutput(win, (win, inputIndex, outputDoneFn) => {
        const targetFile = this._getTargetFile(inputIndex)
        this._generateOutput(win, targetFile, outputDoneFn)
      })
    }
  }
  ...
  _renderAndCollectOutput (win, generateFunc) {
    ...
    this.input.forEach((uriPath, i) => {
      windowEvents.push((outputDoneFn) => this._captureWindow(uriPath, win, generateFunc, outputDoneFn, i))
    })
   ...
   * @param {Number} [inputIndex=0] If multiple input uris are being generated, this is the index of the current one.
   * @private
   */
  _captureWindow (uriPath, win, generateFn, outputDoneFn, inputIndex = 0) {
    this._initializeWindowForResource(this.orientations[inputIndex] === 'landscape')
    const generateFunction = generateFn.bind(this, win, inputIndex, outputDoneFn)
   ...
   * @param window
   * @param outputFile
   * @param {Function} done callback that is invoked when the output file is complete (or errored)
   *
   * @private
   */

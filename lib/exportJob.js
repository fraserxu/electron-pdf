...
class ExportJob extends EventEmitter {
   ...
   *   PDF
   * @param {array} [options.orientation] must correlate to the input array and
   * contain a valid orientation ('landscape','portrait').  If not provided or
  ...
  constructor (input, output, args, options) {
    ...
    const validOrientation = _.size(this.options.orientation) === _.size(this.input)
  ...
  render () {
    ...
    this.input.forEach((uriPath, i) => {
      windowEvents.push((pageDone) => {
        this._initializeWindowForResource(this.orientation[i] === 'landscape')
  ...
  _generateOutput (window, outputFile, done) {
    if (!this.generated) {
      ...
      // window based on the orientation, which can be updated by the client
      // using changeArgValue
      WindowTailor.setWindowDimensions(this.window, this.args.pageSize, this.args.landscape)

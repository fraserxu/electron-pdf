...
if (electron.ipcMain) { // Not available or needed for tests
  electron.ipcMain.on('process-stats', (event, stats) => {
    const { messageId } = stats
    const promise = ipcMessagePromises[messageId]
    if (promise) {
      debugLogger(`resolving promise for messageId: ${messageId}`)
      promise(stats)
      delete ipcMessagePromises[messageId]
    }
    // TODO: bubble this up instead of logging here
    logger('renderer-process-stats', JSON.stringify(stats, undefined, 2))
...
class ExportJob extends EventEmitter {
  ...
  _triggerProcessStats (event) {
    ...
    debugLogger(`invoking ipcApi.eventStats for messageId: ${messageId}`)
    _.invoke(this.window, 'webContents.executeJavaScript', `ipcApi.eventStats('${messageId}','${event}')`)

...
class ExportJob extends EventEmitter {
   ...
   * @param window The `job-complete` event includes a 'window' property so
   *   that this argument can be provided on subsequent render requests.  If a window
   *   is provided it will be resized to match the dimensions specified for this job.
   */
  render (window) {
    logger('render starting...')
    const args = this.args

    const win = window || this._launchBrowserWindow(args)
    if( window ){
      const pageDim = this._getPageDimensions(args.pageSize, args.landscape)
      win.setSize(pageDim.x, pageDim.y)
      this.emit('window.resize', { width: pageDim.x, height: pageDim.y})
    }

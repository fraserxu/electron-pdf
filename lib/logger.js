 ...
 * }
 *
 * @param {Object} instance An instance (typically the caller) for which
 * to assign the loggers to. (e.g. Instance level loggers for a single exportJob)
 */
function set (loggerObj, instance) {
  if (loggerObj) {
  ...
  if (instance) {
    instance.error = loggers[LEVELS.error]
    instance.info = loggers[LEVELS.info]
    instance.debug = loggers[LEVELS.debug]
  }
}
